#!/zfswh1/BC_RD_P0/P17Z12000N0108/wangguangya/bin/anaconda3/bin/python
'''
...
def genebody_percentile(refbed, mRNA_len_cut = 100):
	...
	for line in open(refbed,'r'):
		try:
			...
			geneID = '_'.join([str(j) for j in (chrom, tx_start, tx_end)])
		...
		mRNA_len =0
		flag=0
		#for st,end in zip(tx_start,tx_end):
		gene_all_base.extend(list(range(tx_start+1,tx_end)))		#1-based coordinates on genome
	...
	#print(g_percentiles, file=sys.stderr) ## WGY
